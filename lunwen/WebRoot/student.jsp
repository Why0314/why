  ...
  <head>
   ...
   <body>
    <script type="text/javascript">
		 function search()
			 {
				window.location.href='studentcl.jsp?key='+document.getElementById('key').value;
			 }
		 </script>
<!--
...
<div style="width:100%; background-image:url('image/beijing4.jpg')">
	<center>
		<table  width="100%">
			...
			<% if (request.getSession().getAttribute("zhanghao")==null){ %>
					...
					<!-- <a style="text-decoration:none; font-size:10px; 	" href="gaimi.jsp">修改密码</a> -->		
						<%} %>
				<td>
					<a style="text-decoration:none; font-size:23px; 	" href="daoshicl.jsp">导师管理</a>
				</td>
				<td>
					<a style="text-decoration:none; font-size:23px; font-weight:bold	" href="studentcl.jsp">学生管理</a>
				</td>	
				<td>
		...
		<a href="addStu.jsp" style="color:red">添加学生</a>
        ...
        <tr>
         ...
         <%
						...
						ArrayList<Student> us=(ArrayList<Student>)request.getAttribute("why");//从request里面取出的数据默认是Object类型，所以要强制转化成之前存储的集合类型
						for(int n=0;n<us.size();n++){
							Student u=us.get(n);//循环里面使用n当做下表来取集合对应位置上的对象
						
						%>
						
						<%if(request.getAttribute("updateId")!=null&&u.getStuBianhao()==Integer.parseInt(request.getAttribute("updateId").toString())){ %>
        ...
        <td><%=u.getStuBianhao() %></td>
         <td><%=u.getStuXuehao() %></td>
        <td><%=u.getStuName() %></td>
        <td><%=u.getStuXingbie() %></td>
        <td><%=u.getStuZhuanye() %></td>
        <td><%=u.getStuPassword() %></td>
        <td><%=u.getStuLunwen() %></td>
        <td><%=u.getStuDaoshi() %></td>
        <td>
         <a href="loadoneStu.jsp?bianhao=<%=u.getStuBianhao() %>">修改</a>
         <a href="javascript:deleteStu(<%=u.getStuBianhao() %>)">删除</a>
         </td>
          ...
          <%} %>
                 ...
                 <script type="text/javascript">
                 function deleteStu(bianhao){
	                 if(window.confirm('确认删除吗')){
	                 	window.location.href="deleteStu.jsp?bianhao="+bianhao;
	                 }
                 }
                 </script>
