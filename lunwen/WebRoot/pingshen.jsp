  ...
  <head>
   ...
   <body>
    <script type="text/javascript">
		 function search()
			 {
				window.location.href='pingshencl.jsp?key='+document.getElementById('key').value;
			 }
		 </script>
<!--
			...
			<% if (request.getSession().getAttribute("user")==null){ %>
					...
					<%}else{ %>
						欢迎您：<%=request.getSession().getAttribute("user") %>!
							<a style="text-decoration:none; font-size:10px; coloc:blue	" href="indexPs.jsp">注销</a>
...
<input type="text"     id="key"        value="${requestScope.key }"/>
        ...
        <td><%=u.getChengji() %></td>
         <td>
         <a href="loadonePaperPs.jsp?bianhao=<%=u.getBianhao() %>">评分</a>
