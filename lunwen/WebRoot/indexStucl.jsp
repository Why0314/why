...
//1.获取上一个页面上表单里面的数据
String stuXuehao=request.getParameter("stuXuehao");//用request对象获取上一个页面的参数
...
ResultSet  rs=sta.executeQuery("select * from  student where stuXuehao='"+stuXuehao+"' and stuPassword='"+stuPassword+"'");

//3.根据查询的结果，如果存在，则跳转到首页 ，不存在跳回到登陆页面

if(rs.next()){
	request.getSession().setAttribute("stuXuehao",stuXuehao);
	response.sendRedirect("xueshengcl.jsp");
}else
{
